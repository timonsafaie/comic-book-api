...
(function() {
  var AsyncUtil, CJSON, DATA, DEBUG, FileUtil, HOMEDIR, LIB_COV, LIB_DIR, ObjectUtil, StringUtil, fs, magic, mkdirp, mmmagic, path, ref, remove,
  ...
  FileUtil = (function() {
    ...
    FileUtil.ls = function(dir, options, callback) {
      ...
      recurse = StringUtil.truthy_string((ref1 = options.recurse) != null ? ref1 : false);
      ...
      return fs.readdir(dir, function(err, files) {
        ...
        if ((err != null) || (files == null)) {
          ...
          return AsyncUtil.for_each_async(files, action, function() {
            if (!recurse) {
    ...
    FileUtil.load_json = function(file, options, callback) {
      return FileUtil.load_json_file(file, options, callback);
    };

    FileUtil.load_json_file = function(file, options, callback) {
      if (typeof options === 'function' && (callback == null)) {
        callback = options;
        options = null;
      }
      ...
      return fs.readFile(file, function(err, data) {
        ...
        if (err != null) {
          ...
          try {
            str = data.toString();
            if (options.allow_comments === false) {
              parsed = JSON.parse(str);
            } else {
              parsed = CJSON.parse(str, null, options.strip_comments !== false);
            }
    ...
    FileUtil.load_json_file_sync = function(file, options) {
      var err, parsed, str;
      if (typeof options === 'boolean') {
        options = {
          ignore_errors: options
    ...
    FileUtil.load_json_stdin_sync = function(end_byte, buffer_size, options) {
      var err, parsed, str;
      if (end_byte == null) {
        end_byte = "\x04";
      }
      ...
      try {
        str = FileUtil.read_stdin_sync(end_byte, buffer_size).toString();
        if (options.allow_comments === false) {
          parsed = JSON.parse(str);
        } else {
          parsed = CJSON.parse(str, null, options.strip_comments !== false);
    ...
    FileUtil.get_extension = function(filename) {
      var ext, ref1;
      if (filename != null) {
        ext = (ref1 = StringUtil.blank_to_null(path.extname(filename))) != null ? ref1 : filename;
        if (/^\./.test(ext)) {
    ...
    FileUtil.add_to_mime_to_ext_map = function() {
      ...
      return FileUtil._mime_to_ext_map = ObjectUtil.merge.apply(ObjectUtil, [FileUtil.get_mime_to_ext_map()].concat(slice.call(mappings)));
    };
    ...
    FileUtil.add_to_ext_to_mime_map = function() {
      ...
      return FileUtil._ext_to_mime_map = ObjectUtil.merge.apply(ObjectUtil, [FileUtil.get_ext_to_mime_map()].concat(slice.call(mappings)));
    };
